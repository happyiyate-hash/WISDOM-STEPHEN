...
export const MyTokensView: React.FC<MyTokensViewProps> = ({
  ...
  return (
    <div className="space-y-2.5 text-white font-sans animate-in fade-in duration-200 pb-2">
      {/* 1. Top Summary Card / Metrics Banner (Compact Reduced Sizes) */}
      <div className="bg-[#0B0E17] border border-zinc-800/80 rounded-xl p-2.5 shadow-md flex items-center justify-between">
        {/* Left Metric: Total Tokens Added */}
        <div className="flex items-center space-x-2.5">
          {/* Green Circle with 3D Cube / Box Icon */}
          <div className="w-8 h-8 rounded-full bg-[#15803D]/20 border border-[#22C55E]/40 flex items-center justify-center text-[#4ADE80] shrink-0">
            <Box className="w-3.5 h-3.5 text-[#4ADE80]" />
          </div>

          <div>
            <div className="text-[9px] text-zinc-400 font-medium uppercase tracking-wider">Total Tokens Added</div>
            <div className="text-sm font-black text-white tracking-tight mt-0.5">{totalTokenCount}</div>
            <div className="text-[8px] text-zinc-500 font-normal">Across all networks</div>
          </div>
        </div>

        {/* Right Metric: Total Value (USD) */}
        <div className="text-right">
          <div className="text-[9px] text-zinc-400 font-medium uppercase tracking-wider">Total Value (USD)</div>
          <div className="text-xs sm:text-sm font-black text-white font-mono tracking-tight mt-0.5">
            $12,450.75
          </div>
          <div className="text-[8.5px] font-bold text-[#4ADE80] mt-0.5 flex items-center justify-end gap-0.5 font-mono">
            <span>+18.25%</span>
            <span className="text-zinc-500 font-normal">7d</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-1.5">
        {/* Search Field */}
        <div className="flex-1 bg-[#0B0E17] border border-zinc-800/80 rounded-lg px-2.5 py-1.5 flex items-center space-x-2 focus-within:border-[#22C55E]/50 transition-colors">
        ...
        <button
          ...
          className={`px-2.5 py-1.5 bg-[#0B0E17] hover:bg-zinc-800/80 border rounded-lg text-[10.5px] font-semibold flex items-center space-x-1 transition-colors cursor-pointer shrink-0 ${
